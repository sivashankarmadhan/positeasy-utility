...
import { get, isArray, isEmpty, map, set, startsWith } from 'lodash';
...
function AddCustomerInfo({
  ...
  const getContactNumberWith91 = (contactNumber) => {
    return contactNumber.length === 10 ? `91${contactNumber}` : contactNumber;
  };
  ...
  const defaultValues = {
    ...
    contactNumber: getContactNumberWithout91(get(customerInfo, 'contactNumber', '')) || '',
  ...
  const onSubmit = async (data, isNotPostCustomerData) => {
    const contactNumberWith91 = getContactNumberWith91(get(data, 'contactNumber'));
