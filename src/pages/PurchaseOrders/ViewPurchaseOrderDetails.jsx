...
const ViewPurchaseOrderDetails = () => {
  ...
  const getCurrentStatusIndex = findIndex(PURCHASE_ORDER_STATUS_FOR_BTN_GRP, (_item) => {
  ...
  const changeStatusWithAlert = ({ status }) => {
    const isNotRevert = [
      ...
      PURCHASE_ORDER_STATUS.RETURN,
      PURCHASE_ORDER_STATUS.CLOSE,
    ].includes(status);
    ...
    const alertDialogInformation = {
      ...
      subtitle: `${isNotRevert ? 'You cannot revert the action.' : ''} ${
        PURCHASE_ORDER_STATUS.CANCELLED !== status
  ...
  const isPartiallyPaidStatus =
    get(PURCHASE_ORDER_STATUS_FOR_BTN_GRP, `${getCurrentStatusIndex}.name`) ===
  ...
  const postPayment = async (data, reset) => {
    try {
      ...
      if (get(data, 'type') === PURCHASE_ORDER_PAYMENT_TYPE.FULL_PAYMENT) {
        await changeStatus(PURCHASE_ORDER_STATUS.CLOSE);
      } else if (
        get(data, 'type') === PURCHASE_ORDER_PAYMENT_TYPE.PARTIAL &&
        currentStatus !== PURCHASE_ORDER_STATUS.PARTIALLY_PAID
      ) {
        await changeStatus(
          get(PURCHASE_ORDER_STATUS_FOR_BTN_GRP, `${getCurrentStatusIndex + 1}.name`)
        );
      }
      getPurchaseOrderDetails();
    } catch (e) {
  ...
  return (
    <Box m={2}>
            ...
            <Stack flexDirection="row" gap={2}>
              {![
                ...
                !isAtleastStockAddedOne && (
                  <Button
                    ...
                    color="error"
                    onClick={() => {
                      changeStatusWithAlert({ status: PURCHASE_ORDER_STATUS.CANCELLED });
                    }}
                  >
                    Cancel
                  </Button>
              ...
              {![
                ...
                <ButtonGroup variant="contained">
                  ...
                  <Button
                    sx={
                      getCurrentStatusIndex === PURCHASE_ORDER_STATUS_FOR_BTN_GRP.length - 1
                        ? {
                            cursor: 'not-allowed',
                          }
                        : {}
                    ...
                    size="medium"
                    onClick={() => {
                      if (getCurrentStatusIndex !== PURCHASE_ORDER_STATUS_FOR_BTN_GRP.length - 1) {
              ...
              {/* {![PURCHASE_ORDER_STATUS.CANCELLED, PURCHASE_ORDER_STATUS.RETURN].includes(
                currentStatus
              ) && (
                <Button
                  variant="contained"
                  color="warning"
                  onClick={() => {
                    changeStatus(PURCHASE_ORDER_STATUS.RETURN);
                  }}
                >
                  Return
                </Button>
      ...
      {isOpenProductModal?.status && (
        <AddProductOrRawMaterialToStore
          isStorePurchase={location?.state?.storePurchase }
