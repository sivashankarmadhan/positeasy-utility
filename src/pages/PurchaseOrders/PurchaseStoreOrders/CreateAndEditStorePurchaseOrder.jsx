...
import StorePurchaseOrderBasicDetailsFields from 'src/sections/PurchaseOrders/PurchaseOrderStores/StorePurchaseOrderBasicDetailsFields';
import StorePurchaseOrderProductList from 'src/sections/PurchaseOrders/PurchaseOrderStores/StorePurchaseOrderProductList';
import INTENT_API from 'src/services/IntentService';

const CreateAndEditStorePurchaseOrder = () => {
  ...
  const schema = Yup.object().shape({
    storeName: Yup.object().shape({
      label: Yup.string().required(REQUIRED_CONSTANTS.STORE_IS_REQUIRED),
      id: Yup.string().required(REQUIRED_CONSTANTS.STORE_IS_REQUIRED),
    }),
  ...
  useEffect(()=> {
    if(selectedTab === "byPass") {
      setValue('status', PURCHASE_ORDER_STATUS.TRANSIT )
    } else if(selectedTab === "normal") {
      setValue('status', PURCHASE_ORDER_STATUS.OPEN )
    }
  }, [selectedTab])
  ...
  const onSubmit = async (data) => {
    ...
    const ordersList = map(productList, (_item) => {
      let list = {
        ...omit(_item, 'rate'),
        name: get(_item, 'name'),
      ...
      if (!get(_item, 'counterId.id')) {
        list = omit(list, 'counterId');
      } else {
        set(list, 'counterId', get(_item, 'counterId.id'));
      }
      return list;
    });

    const storeReferenceData = storeReferenceList.find(
      (item) => item.storeId === get(data, 'storeName.id')
    )?.storeReference;
    ...
    const payload = {
      ...
      receiverReference: storeReferenceData,
      ...([PURCHASE_ORDER_PAYMENT_TYPE.FULL_PAYMENT, PURCHASE_ORDER_PAYMENT_TYPE.PARTIAL].includes(
        get(watchPaymentType, 'id')
      )
        ? { mode: get(data, 'mode') }
        : {}),
      status: selectedTab === "byPass" ? ORDERS_STATUS_CONSTANTS.TRANSIT : get(data, 'status'),
    };

    setIsLoading(true);
    try {
     selectedTab === "byPass" ? await INTENT_API.addDirectPurchaseIntentOrder(payload) : await INTENT_API.addPurchaseIntentOrder(payload);
      toast.success(SuccessConstants.ADDED_PURCHASE_ORDER_SUCCESSFULLY);
      navigate(PATH_DASHBOARD.purchases.viewPurchaseOrders, {
        state: { isStorePurchase: true },
      });
    } catch (e) {
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Stack pl={2}>
        ...
        >
          <Tab value="normal" label="Normal Request" />
          <Tab value="byPass" label="Bypass Request" />
        </Tabs>
      </Stack>
      <Box
        sx={{
      ...
      >
        ...
        <StorePurchaseOrderProductList
          ...
          receiverStoreId={watchStorename?.id}
        />
      </Box>
      <Stack
...
export default CreateAndEditStorePurchaseOrder;
