import {
  ...
  export default function DailySaleNotificationDialog(props) {
  ...
  console.log('emailList', emailList);
    ...
    const handleDelete = (_index) => {
      ...
      const filterData = filter(emailList.filter(email => email !== ''), (contact, index) => {
        return index !== _index;
      });
        ...
        <Card sx={{ p: 2, width: { xs: 360, sm: 500 }, overflowY: 'auto' }}>
          ...
          <Stack flexDirection={'row'} gap={1} marginBottom={1}>
            <TextField
              ...
              disabled={emailList?.length === 5 && !isEditFlow}
            />
          ...
          <Stack mt={2}>
            ...
            >
              {map(emailList.filter(email => email !== ''), (_contact, _index) => {
                return (
                  <Stack
                    sx={{
                      p: 1.5,
